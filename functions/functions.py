...
def get_stock_data(data_rif, array, time, difference, is_save,folder1, folder2, folder_result):
    ...
    if time == 'giornaliero':
        ...
        for name in array: 
            ...
            if not stock_data.empty:
                ...
                stock_data1.to_csv(f'{folder1}/{name}_giornaliero.csv') if is_save == True else None
                ...
                stock_data2.to_csv(f'{folder2}/{name}_giornaliero_2.csv') if is_save == True else None
        ...
        for name in array: 
            ...
            if not stock_data1.empty:            
                stock_data1.to_csv(f'{folder1}/{name}_settimanale.csv') if is_save == True else None
                min1=stock_data1['Low'].min()
                ...
                stock_data2.to_csv(f'{folder2}/{name}_settimanale_2.csv') if is_save == True else None
                min2=stock_data2['Low'].min()
        ...
        for name in array: 
            ...
            # Verifica se stock_data è vuoto
            if not stock_data1.empty:            
                stock_data1.to_csv(f'{folder1}/{name}_mensile.csv') if is_save == True else None
                min1=stock_data1['Low'].min()
                open1=stock_data1['Open'].head(1).values[0]
                close1=stock_data1['Close'].tail(1).values[0]
                ...
                # Identificazione del min2 (secondo periodo di comparazione)
                stock_data2 = download_data(name, start_date2, end_date2, interval)            
                stock_data2.to_csv(f'{folder2}/{name}_mensile_2.csv') if is_save == True else None
                min2=stock_data2['Low'].min()
                open2=stock_data2['Open'].head(1).values[0]
                close2=stock_data2['Close'].tail(1).values[0]
